...
std::vector<int> pos_gen::genFrom(Node src) {
    std::srand(std::time(NULL));

    int length = getRandomLength();
    int dir = getRandomDir();

    std::vector<int> next_pos = src.get_pos();
    float curClosestPos;
    ...
    do {
        ...
        curClosestPos = getClosestPos(next_pos);
    }while(curClosestPos < 1.0f);

    return next_pos;
}
//PQ = d = √ [(x2 – x1)2 + (y2 – y1)2 + (z2 – z1)2].
float d(std::vector<int> from, std::vector<int>to) {
...
float pos_gen::getClosestPos(std::vector<int> pos) {
    float ans = INF;
